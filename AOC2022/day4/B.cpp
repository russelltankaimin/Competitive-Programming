...
void solve(){
  ...
  int pp;
  int called;
  vector<int> boardsWin;
  unordered_map<int, int> winIndex;
  vector<int> calledVec;
  vector<vector<vector<bool>>> visited = vector<vector<vector<bool>>>(100, vector<vector<bool>>(5, vector<bool>(5, false)));
  for (int r = 0; r < 100; r++) {
    for (int i = 0; i < 100; i++) {
      for (int j = 0; j < 5; j++) {
        for (int k = 0; k < 5; k++) {
          if (winIndex.count(i) == 0) {
            visited[i][j][k] = visited[i][j][k] | (nums[r] == grids[i][j][k]); 
          }
        }
      }
      bingo = checkBoard(visited[i]);
      if (bingo && winIndex.count(i) == 0) {
        boardsWin.push_back(i);
        winIndex[i] = 1;
        calledVec.push_back(nums[r]);
        called = nums[r];
      }
    }
  }
  set<int> s(ALL(boardsWin));
  assert(boardsWin.size() == s.size());
  pp = boardsWin.back();
  called = calledVec.back();
  int score = 0;
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 5; j++) {
